...
# ====== 读取数据 ======
csv_file = "outputs/adult_accs.csv"
df = pd.read_csv(csv_file, nrows=30)
...
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "adult_test_accuracy_plot.png"),
            dpi=300, bbox_inches="tight")
plt.show()


# =========================
# 2️⃣ Training Accuracy 图
# =========================
plt.figure(figsize=(8, 6))

plt.plot(df["epoch"], df["product_train_accuracy"] * 100,
         color="red", linewidth=2, linestyle="-",
         label="proposed product noise")

plt.plot(df["epoch"], df["gaussian_train_accuracy"] * 100,
         color="blue", linewidth=2, linestyle="--",
         label="classical Gaussian noise")

plt.plot(df["epoch"], df["baseline_train_accuracy"] * 100,
         color="black", linewidth=2, linestyle="-.",
         label="non-private baseline")
...
plt.tight_layout()
plt.savefig(os.path.join(save_dir, "adult_train_accuracy_plot.png"),
            dpi=300, bbox_inches="tight")
plt.show()